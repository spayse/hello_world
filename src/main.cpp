...
#if defined(NDEBUG)
#error "Hello cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSIRAJ spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zSIRAJ spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zSIRAJ spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 300000 * COIN; // Developer Fund -- Block 1
    } else if (nHeight < 51840 && nHeight > 0) {
        nSubsidy = 4 * COIN; // 90 days -- Emissions: 207.356, Cumulative: 507.356
    } else if (nHeight < 103680 && nHeight >= 51840) {
        nSubsidy = 6 * COIN; // 90 days -- Emissions: 311.040, Cumulative: 818.396
	} else if (nHeight < 155520 && nHeight >= 103680) {
        nSubsidy = 10 * COIN; // 90 days -- Emissions: 518.400, Cumulative: 1.336.796
	} else if (nHeight < 210240 && nHeight >= 155520) {
        nSubsidy = 8 * COIN; // 95 days -- Emissions: 437.760, Cumulative: 1.774.556
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 210240) {
        nSubsidy = 6 * COIN; // 2 years -- Emissions: 2.522.880, Cumulative: 4.297.436
    } else if (nHeight <= 682559 && nHeight > Params().LAST_POW_BLOCK()) {
        nSubsidy = 2 * COIN; // 90 days -- Emissions: 103.680, Cumulative: 4.401.116
    } else if (nHeight <= 734399 && nHeight >= 682560) {
        nSubsidy = 1.95 * COIN; // 90 days -- Emissions: 101.080, Cumulative: 4.502.196
    } else if (nHeight <= 786239 && nHeight >= 734400) {
        nSubsidy = 1.90 * COIN; // 90 days -- Emissions: 98.196, Cumulative: 4.600.392
    } else if (nHeight <= 838079 && nHeight >= 786240) {
        nSubsidy = 1.85 * COIN; // 90 days -- Emissions: 95.904, Cumulative: 4.696.296
    } else if (nHeight <= 889919 && nHeight >= 838080) {
        nSubsidy = 1.80 * COIN; // 90 days -- Emissions: 93.312, Cumulative: 4.789.608
    } else if (nHeight <= 941759 && nHeight >= 889920) {
        nSubsidy = 1.75 * COIN; // 90 days -- Emissions: 90.720, Cumulative: 4.880.328
    } else if (nHeight <= 993599 && nHeight >= 941760) {
        nSubsidy = 1.70 * COIN; // 90 days -- Emissions: 88.128, Cumulative: 4.968.456
    } else if (nHeight <= 1045439 && nHeight >= 993600) {
        nSubsidy = 1.65 * COIN; // 90 days -- Emissions: 85.536, Cumulative: 5.053.992
    } else if (nHeight <= 1097279 && nHeight >= 1045440) {
        nSubsidy = 1.50 * COIN; // 90 days -- Emissions: 77.760, Cumulative: 5.131.752
	} else if (nHeight <= 1149119 && nHeight >= 1097280) {
        nSubsidy = 1.25 * COIN; // 90 days -- Emissions: 64.800, Cumulative: 5.196.552
    } else if (nHeight >= 1149120) {
        nSubsidy = 1 * COIN; // 23 years -- Emissions: 4.835.520, Cumulative: 10.032.072
    } else {
        nSubsidy = 0 * COIN;
    }
    return nSubsidy;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= 51840) {
        ...
        ret = blockValue / 1.7;
    } else if (nHeight < 210240 && nHeight >= 103680) {
        ret = blockValue / 1.6;
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 210240) {
        ret = blockValue / 1.5;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Hello
...
void RecalculateZSIRAJSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSIRAJ supply
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zSIRAJ with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSIRAJSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SIRAJ to zSIRAJ
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSIRAJ spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSIRAJ serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSIRAJ mints and %d zSIRAJ spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Hello: We use certain sporks during IBD, so check to see if they are
