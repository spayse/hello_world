                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSIRAJ ought to be enough for anybody." - Bill Gates, 2017
    ui->zSIRAJpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSIRAJSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSIRAJ->setEnabled(false);
        ui->pushButtonMintzSIRAJ->setToolTip(tr("zSIRAJ is currently disabled due to maintenance."));

        ui->pushButtonSpendzSIRAJ->setEnabled(false);
        ui->pushButtonSpendzSIRAJ->setToolTip(tr("zSIRAJ is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSIRAJpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSIRAJ_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSIRAJ is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSIRAJ...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSIRAJ in ") + 
...
void PrivacyDialog::on_pushButtonSpendzSIRAJ_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSIRAJ is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSIRAJ
        sendzSIRAJ();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSIRAJ
    sendzSIRAJ();
}

void PrivacyDialog::on_pushButtonZSIRAJControl_clicked()
{
    ZSIRAJControlDialog* zSIRAJControl = new ZSIRAJControlDialog(this);
    zSIRAJControl->setModel(walletModel);
    zSIRAJControl->exec();
}

void PrivacyDialog::setZSIRAJControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSIRAJSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSIRAJ()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid hello Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSIRAJpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSIRAJ is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SIRAJ </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSIRAJ</b>";
    ...
    // use mints from zSIRAJ selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSIRAJControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSIRAJControlDialog::GetSelectedMints();
    }
    ...
    // Clear zSIRAJ selector in case it was used
    ZSIRAJControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSIRAJ Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " SIRAJ, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSIRAJ Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSIRAJ </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSIRAJ "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSIRAJ "));
    ui->labelzSIRAJAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSIRAJSyncStatus->setVisible(fShow);
}
