...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the ZND and zZND money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ZND/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ZND/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ZND/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-backupzznd=<n>", strprintf(_("Enable automatic wallet backups triggered after each zZND minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizehelloamount=<n>     " + strprintf(_("Keep N ZND anonymized (default: %u)"), 0) + "\n";
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZZNDMinted();
                        RecalculateZZNDSpent();
                    }
                    RecalculateZNDSupply(1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZZNDBackups = GetBoolArg("-backupzznd", true);
        pwalletMain->setZZNDAutoBackups(fEnableZZNDBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    if (nAnonymizehelloAmount < 2) nAnonymizehelloAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1ZND+1000 == (.1ZND+100)*10
       10ZND+10000 == (1ZND+1000)*10
    */
