                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zZND ought to be enough for anybody." - Bill Gates, 2017
    ui->zZNDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzZNDSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzZND->setEnabled(false);
        ui->pushButtonMintzZND->setToolTip(tr("zZND is currently disabled due to maintenance."));

        ui->pushButtonSpendzZND->setEnabled(false);
        ui->pushButtonSpendzZND->setToolTip(tr("zZND is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zZNDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzZND_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZND is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zZND...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zZND in ") + 
...
void PrivacyDialog::on_pushButtonSpendzZND_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZND is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zZND
        sendzZND();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zZND
    sendzZND();
}

void PrivacyDialog::on_pushButtonZZNDControl_clicked()
{
    ZZNDControlDialog* zZNDControl = new ZZNDControlDialog(this);
    zZNDControl->setModel(walletModel);
    zZNDControl->exec();
}

void PrivacyDialog::setZZNDControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzZNDSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzZND()
{
    ...
    double dAmount = ui->zZNDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zZND is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ZND </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zZND</b>";
    ...
    // use mints from zZND selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZZNDControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZZNDControlDialog::GetSelectedMints();
    }
    ...
    // Clear zznd selector in case it was used
    ZZNDControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zZND Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ZND, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zZND Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zZND </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zZND "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zZND "));
    ui->labelzZNDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzZNDSyncStatus->setVisible(fShow);
}
